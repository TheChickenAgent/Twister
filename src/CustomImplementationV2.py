##############
# ESPECIALLY DESIGNED FOR ONE K-WAY CLASSIFIER (CE) AND ONE MATCHING CLASSIFIER (BCE)
##############
import torch
import numpy as np
...
class CustomImplementation:
    ...
    V2: This version is especially designed for one k-way classifier (CE) and one matching classifier (BCE)
    ...
    def __init__(
        ...
        self.loss_BCE = torch.nn.BCELoss(weight=class_weights)
    ...
    def train_loop(
        ...
        for batch, (X, y) in enumerate(dataloader):
            ...
            combined_loss = loss_ce + loss_bce
